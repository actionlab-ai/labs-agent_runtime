...
// 中文业务注释：Flow.Run 是 LLM Agent 的循环引擎。它会反复执行 runOneStep，直到出现最终回复、转交、错误或上下文结束。
...
// 中文业务注释：runOneStep 表示一次“模型步”：加工请求 -> 调模型 -> 产出模型事件 -> 如有工具调用则执行工具并产出工具响应事件。
func (f *Flow) runOneStep(ctx agent.InvocationContext) iter.Seq2[*session.Event, error] {
	return func(yield func(*session.Event, error) bool) {
		...
		// 中文业务注释：先创建空的 LLMRequest，随后由 RequestProcessors 逐步填入指令、历史、工具声明、输出 Schema 等内容。
		req := &model.LLMRequest{
			Model: f.Model.Name(),
		}

		// Preprocess before calling the LLM.
		// 中文业务注释：请求预处理流水线。这里是 Prompt/工具/历史上下文真正被拼入 LLMRequest 的地方。
		...
		// Calls the LLM.
		// 中文业务注释：调用模型。callLLM 内部会执行插件/回调、记录遥测，并兼容流式与非流式响应。
		for resp, err := range f.callLLM(ctx, req, stateDelta, artifactDelta) {
			...
			// 中文业务注释：如果模型返回 function_call，这里会真正执行工具，并生成 function_response 事件给下一轮模型消费。
...
// 中文业务注释：preprocess 是请求加工管线入口。不同 processor 按顺序修改同一个 LLMRequest。
...
// 中文业务注释：callLLM 负责模型调用前后回调、错误恢复回调、插件拦截，以及把模型响应包装成带 eventID 的内部对象。
...
// 中文业务注释：generateContent 是唯一真正调用底层 model.LLM 的地方，同时包裹 OpenTelemetry span 和请求/响应日志。
...
// 中文业务注释：把模型原始响应转换成 Session Event。此处会补齐 function_call ID、设置 Author/Branch，并标记长任务工具调用。
...
// 中文业务注释：执行模型请求的所有工具调用。多个 function_call 会并发运行，最后合并为一个 function_response event，减少后续模型上下文碎片。
...
// 中文业务注释：单个工具的完整调用链：插件 beforeTool -> Agent beforeTool -> 真正 Run -> 错误回调 -> afterTool，最后统一返回 map。
