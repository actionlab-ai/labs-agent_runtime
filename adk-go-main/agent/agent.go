...
// 中文业务注释：基础 Agent 构造器。自定义 Agent 只需要提供 Run 函数，就能接入 Runner/Session/Plugin/Workflow 体系。
...
// 中文业务注释：所有 Agent 的统一执行外壳。它负责开启遥测、构造当前 Agent 的上下文、执行 before/after 回调，并给事件补 Author。
func (a *agent) Run(ctx InvocationContext) iter.Seq2[*session.Event, error] {
	return func(yield func(*session.Event, error) bool) {
		...
		// 中文业务注释：进入具体 Agent 逻辑。对 LLMAgent 来说这里会进入 internal/llminternal.Flow；对工作流 Agent 来说会调度子 Agent。
