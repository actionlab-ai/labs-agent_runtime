...
// 中文业务注释：创建 Session。这里不仅保存 session 私有状态，还会把 app:/user: 前缀的状态拆入全局/用户级状态池。
...
// 中文业务注释：读取 Session。返回前会重新合并 appState、userState、sessionState，并可按最近条数/时间过滤事件。
...
// 中文业务注释：追加事件是 Session 管理的核心。每个非 partial Event 都会进入历史，同时根据 StateDelta 更新不同作用域的状态。
func (s *inMemoryService) AppendEvent(ctx context.Context, curSession Session, event *Event) error {
	...
	// 中文业务注释：流式 partial 只用于实时展示，不进入长期历史；最终非 partial 事件才是可回放、可持久化的事实。
	if event.Partial {
		return nil
	}
	...
	// 中文业务注释：StateDelta 是“事件驱动状态更新”的入口。这里把 app:/user:/session 普通 key 拆分到对应存储层。
...
// 中文业务注释：temp: 状态只在当前 invocation 内有效，不能污染长期会话历史，因此追加事件前会裁剪。
...
// 中文业务注释：把 Event 上携带的 StateDelta 合并到当前 session 可见状态，让后续 prompt 模板和工具能够读取。
