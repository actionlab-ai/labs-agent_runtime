...
// 中文业务注释：Event 是框架里的“事实记录”。不要把它只理解成聊天消息，它还承载工具调用、工具响应、状态变化、转交、人工确认等运行时信息。
type Event struct {
	model.LLMResponse
...
// 中文业务注释：判断一个事件是否让本轮 Flow 停止。只要还包含 function_call/function_response/partial，通常说明还需要继续下一步模型或工具循环。
...
// 中文业务注释：EventActions 是事件附带的“副作用指令”，例如更新状态、保存 Artifact、请求人工确认、转交给其他 Agent。
