...
// 中文业务注释：New 把一个强类型 Go 函数包装成 LLM 工具。参数/返回值 Schema 会自动推导，模型据此知道如何传参。
...
// 中文业务注释：Run 是工具执行入口。它完成 map 参数 -> 强类型参数 -> 业务函数 -> 结构化 map 返回的双向转换，并处理人工确认。
func (f *functionTool[TArgs, TResults]) Run(ctx tool.Context, args any) (result map[string]any, err error) {
	...
	// 中文业务注释：这里才真正进入你的业务函数。前面的逻辑都是框架层参数校验、Schema 转换和确认流程。
